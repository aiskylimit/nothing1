...
def add_model_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--student-model-type", type=str, default=None)
    group.add_argument("--n-gpu", type=int, default=1)
    group.add_argument("--n-nodes", type=int, default=1)
    group.add_argument("--student-model-path", type=str)
    group.add_argument("--student-ckpt-name", type=str)
    group.add_argument("--student-model-fp16", action="store_true")
...
def add_hp_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument('--warmup-ratio', type=float, default=0.05),
...
def add_distillm_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--student-lr", type=float, default=5e-5)
    group.add_argument("--delta-lamda", type=float, default=1.0)
...
def add_peft_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--student-peft-name", type=str, default=None)
    group.add_argument("--student-peft-path", type=str, default=None)
    return parser
...
def get_args():
    ...
    if args.type == "eval_main":
        ...
        save_path = os.path.join(
            args.save,
            (f"{args.ckpt_name}" + f"-{args.peft_name}" if args.peft_name is not None else "" + \
             f"-{args.student_ckpt_name}" + f"-{args.student_peft_name}" if args.student_peft_name is not None else ""),
