...
@torch.no_grad()
def compute_dataset_ppl(dataset, model, tokenizer, batch_size=8, device="cuda"):
    """
    Tính Perplexity (PPL) cho từng sequence trong dataset bằng Student model.
    """
    model.eval()
    model.to(device)
    
    # Đảm bảo tokenizer có pad_token để batching
    ...
    for inputs, masks in tqdm(dataloader, desc="Calculating Perplexity"):
        ...
        # Copy inputs làm labels và mask bỏ phần padding (-100 sẽ bị pytorch loss bỏ qua)
        labels = inputs.clone()
        labels[masks == 0] = -100 
        ...
        # Dịch chuyển logits và labels để dự đoán token tiếp theo (Causal LM)
        shift_logits = logits[..., :-1, :].contiguous()
        shift_labels = labels[..., 1:].contiguous()
        
        # Tính Cross Entropy Loss cho từng token, không tính trung bình gộp (reduction='none')
        loss_fct = torch.nn.CrossEntropyLoss(reduction='none')
        loss = loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))
        loss = loss.view(shift_labels.size())
        
        # Tính trung bình Loss cho từng câu (chỉ chia cho số token thật, bỏ qua pad)
        seq_lens = (shift_labels != -100).sum(dim=1)
        seq_loss = loss.sum(dim=1) / seq_lens
        
        # Perplexity = exp(Loss)
        seq_ppl = torch.exp(seq_loss).cpu().tolist()
        all_ppls.extend(seq_ppl)
...
def main(cfg: DictConfig):
    ...
    log_color("Starting PPL filtering...", title="PPL Filter")
    ppl_scores = compute_dataset_ppl(train_traces, student, tokenizer, batch_size=8, device="cuda")
    train_traces = train_traces.add_column("ppl", ppl_scores)
    p80_threshold = np.percentile(ppl_scores, 80)
    log_color(f"PPL 80th Percentile Threshold: {p80_threshold:.4f}", title="Filter Threshold")
    original_size = len(train_traces)
    train_traces = train_traces.filter(
        lambda x: x["ppl"] <= p80_threshold,
        desc="Filtering out top 20% highest PPL"
    )
    new_size = len(train_traces)
    log_color(f"Filtered dataset from {original_size} to {new_size} samples. Removed {original_size - new_size} noisy traces.", title="Filter Result")
    train_traces = train_traces.remove_columns("ppl")
