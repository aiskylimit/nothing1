...
class StudentMetaAccumulator:
    ...
    def accumulate_and_compute_anti_loss(self, model_batch, no_model_batch, args, logits):
        ...
        # loss = get_distil_loss(args, logits.detach().to(self.student_device), no_model_batch, student_logits)
        loss = self.loss_func(student_logits.float().view(-1, logits.shape[-1]), no_model_batch["label"].view(-1))

        trainable_named_params = {
            n: p for n, p in self.student.named_parameters() if p.requires_grad
        }
        
        trainable_params_list = list(trainable_named_params.values())
        grads = torch.autograd.grad(loss, trainable_params_list, retain_graph=False)
        
        for name, g in zip(trainable_named_params.keys(), grads):
            if name not in self.grad_buffer:
                self.grad_buffer[name] = g.detach()
            else:
                self.grad_buffer[name] += g.detach()
        
        if self.current_step % self.acc_steps == 0:
            return self._compute_loss(args, model_batch, no_model_batch, logits)
        
        return None

    def _compute_loss(self, args, model_batch, no_model_batch, logits):
        # 1. Tính Gradient trung bình
        avg_grads = {k: v / self.acc_steps for k, v in self.grad_buffer.items()}
        
        # Tính Norm để scale LR (trên batch lớn đã ổn định)
        grad_norm = torch.sqrt(sum(g.pow(2).sum() for g in avg_grads.values()))
        max_norm = 1.0
        clip_coef = max_norm / (grad_norm + 1e-6)
        clip_coef = torch.clamp(clip_coef, max=1.0)

        # 2. Tạo Updated Params
        updated_params = {}
        for name, param in self.student.named_parameters():
            if name in avg_grads:
                grad = avg_grads[name]                
                # layer_norm = torch.norm(grad)
                # normalized_grad = grad / layer_norm   
                normalized_grad = grad * clip_coef          
                updated_params[name] = param + args.student_lr * normalized_grad
            else:
                updated_params[name] = param
        
        self.grad_buffer = {}
        
        # ---- Forward 2: loss sau 1 bước SGD giả ----
        kwargs = dict(model_batch)
        kwargs["use_cache"] = False
        with torch.no_grad():
            student_output_2 = torch.func.functional_call(self.student, updated_params, (), kwargs)

        student_output_2_logits = student_output_2.logits.float()
        no_model_batch["label"] = no_model_batch["label"]
        valid_mask = (no_model_batch["label"] != -100).float()
        probs = torch.softmax(student_output_2_logits.detach(), dim=-1)
        ...
        k = 50
        s_topk_logits, s_topk_indices = torch.topk(student_output_2_logits.float(), k, dim=-1)
    ...
    for epoch in range(args.epochs):
        ...
        for it, (model_batch, no_model_batch, gen_data) in enumerate(train_dataloader):
            ...
            if anti_loss is not None:
                ...
                loss = (base_loss + h_base_loss) / 2 + anti_loss                
                logit_adapter.backward(loss)
